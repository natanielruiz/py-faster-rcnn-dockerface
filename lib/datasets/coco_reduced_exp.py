...
class coco_reduced(imdb):
    def __init__(self, image_set, year):
        ...
        keep_cats = ['microwave', 'refrigerator', 'bowl', 'bottle', 'sink', 'sandwich',
        'hot dog', 'fork', 'knife', 'spoon', 'carrot', 'cup', 'pizza']
        # cats = self._COCO.loadCats(self._COCO.getCatIds())
        # self._classes = tuple(['__background__'] + [c for c in keep_cats])
        self._classes = ('__background__', # always index 0
                 'microwave', 'refrigerator', 'bowl', 'bottle', 'sink', 'sandwich',
                 'hot dog', 'fork', 'knife', 'spoon', 'carrot', 'cup', 'pizza')
        self._class_to_ind = dict(zip(self.classes, xrange(self.num_classes)))
        self._class_to_coco_cat_id = dict(zip([c for c in keep_cats],
