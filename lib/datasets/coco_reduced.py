...
class coco_reduced(imdb):
    def __init__(self, image_set, year):
        ...
        # self.keep_cats = ['microwave', 'refrigerator', 'bowl', 'bottle', 'sink', 'sandwich',
        # 'hot dog', 'fork', 'knife', 'spoon', 'carrot', 'cup', 'pizza']
        self.keep_cats = ['bottle', 'carrot', 'fork', 'sandwich']
        cats = self._COCO.loadCats(self._COCO.getCatIds(catNms=self.keep_cats))
        self._classes = tuple(['__background__'] + [c['name'] for c in cats])
        self._class_to_ind = dict(zip(self.classes, xrange(self.num_classes)))
        print self._class_to_ind
        # Custom indices
        # self._class_to_ind = {u'fork': 42, u'sandwich': 43, u'cup': 10, u'hot dog': 14, u'bowl': 3, u'spoon': 38, u'microwave': 21,
        #  '__background__': 0, u'carrot': 44, u'sink': 35, u'bottle': 41, u'refrigerator': 32, u'knife': 18, u'pizza': 29}
        self._class_to_coco_cat_id = dict(zip([c['name'] for c in cats],
                                              self._COCO.getCatIds(catNms=self.keep_cats)))
    ...
    def _load_coco_annotation(self, index):
        ...
        annIds = self._COCO.getAnnIds(imgIds=index, iscrowd=None, catIds=self._COCO.getCatIds(catNms=self.keep_cats))
        ...
        overlaps = np.zeros((num_objs, 45), dtype=np.float32)
